...
uint16_t getPaddingBytes(
    uint16_t currentIndex, SegmentDefinition::Alignment alignment) {
  switch (alignment) {
  ...
  for (const auto& scriptItem : linkScript) {
    if (std::holds_alternative<LinkScriptAddress>(scriptItem)) {
      ...
      if (scriptAddr < currentIndex) {
        // throw std::runtime_error{std::format("Fixed address 0x{:x} from link
        // script leaves insufficient space for earlier sections", scriptAddr)};
        std::runtime_error{
            "Fixed address in link script leaves insufficient space for earlier sections"};
      }

      currentIndex = scriptAddr;
    } else if (std::holds_alternative<LinkScriptSection>(scriptItem)) {
      std::string_view currentSectionName =
          std::get<LinkScriptSection>(scriptItem).name;
      for (auto& unit : units) {
  ...
  for (const auto& unit : units) {
    for (const auto& s : unit.segments) {
      if (s.baseAddress == 0xffff) {
        // throw std::runtime_error{std::format("Section {} defined in {} is
        // missing from link script", s.segmentName, unit.name)};
